 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_cbir.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* cbir.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* cbir.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* cbir (application created by MATLAB Compiler)
		* run_cbir.sh (shell script that can be run 
	...
	All Platforms:            
		* cbir.dll (shared library, file extension 
		  varies by platform)
		* cbir.h (library header file)
		* cbir.ctf (Component Technology File archive,
	...
	* cbir.xml (documentation files)
	* cbir.pdb (program database file - if 
	  DEBUG option selected)
	* cbir.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* cbir.jar (Java class files and compiled M-code)
	...
	* cbir.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* cbir.xla (Optionally created and placed in
